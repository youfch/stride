...
namespace Stride.Assets.Presentation
{
    public sealed class StrideDefaultAssetsPlugin : StrideAssetsPlugin
    {
        ...
        {
#if AVALONIA
            // Skip WPF-specific template loading in Avalonia mode
            return;
#endif
        ...
        public override void InitializeSession(SessionViewModel session)
        {
            ...
            session.SuggestedPackages.Add(new PackageName(typeof(Stride.BepuPhysics.Definitions.DecomposedHulls).Assembly.GetName().Name, new PackageVersion(StrideVersion.NuGetVersion))); // We don't want this solution to reference Bepu directly as it must stay as a plugin, so a hardcoded string will have to do for now
